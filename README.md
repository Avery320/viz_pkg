...
- 腳本（src/viz_pkg/scripts）
  - viz_obj.py：常駐節點；載入 STL、發佈 Marker（/MarkerRes/<id> 與 /MarkerTri/<id>，依 ~marker_mode）、轉發 PlanningScene 至 /planning_scene/scene（latched），啟動即呼叫 /get_planning_scene 發佈初始快照
...
- RViz Marker（依 ~marker_mode）：
  - resource：/MarkerRes/<id>（type=10 MESH_RESOURCE）
...
```
- 只發布 Marker（不加入碰撞，resource 模式）
```bash
rosrun viz_pkg viz_obj.py _mesh_path:=model/3dcp_ws.stl _publish_markers:=true _add_collision:=false _marker_mode:=resource
# 預設 topic：/MarkerRes/<id>
```
